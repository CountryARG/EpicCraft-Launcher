...
def wait_text(value, seconds=90):
    ...
    while time.time()<end:
        ns=nodes()
        if any(n.get('text')==value for n in ns): return ns
        time.sleep(2)
...
def tap_text(value):
    ns=wait_text(value)
    tap(next(n for n in ns if n.get('text')==value))

adb('shell','am','start','-n','com.nebulateam.epiccraft.debug/net.kdt.pojavlaunch.TestStorageActivity')
ns=wait_text('Iniciar sesión')
assert not any(n.get('text')=='Añadir cuenta' for n in ns), 'Cuenta visible antes de login'
...
ns=wait_text('JUGAR')
assert any(n.get('text')=='Añadir cuenta' for n in ns)
...
ns=wait_text('Iniciar sesión')
assert not any(n.get('text')=='Añadir cuenta' for n in ns)
